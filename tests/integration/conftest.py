...
These tests need a live MLDP ecosystem listening on localhost:50051-50053,
however it is started (docker compose, or the services run directly).  They
self-skip when it is absent, but CI deselects them outright with
`-m "not integration"` so a missing backend is never mistaken for a passing run.
